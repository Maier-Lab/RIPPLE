...
.panel_multi_volcano <- function(all_results, sig_results, coef_col, sig_col,
  ...
  for (i in seq_len(nrow(label_set))) {
    volcano_data[
      gene == label_set$gene[i] & cell_type == label_set$cell_type[i],
      show_label := TRUE
    ]
  }
...
.panel_specific_heatmap <- function(all_results, sig_results, gene_spec,
  ...
  .save_pheatmap_pdf(
    ...
    main = paste0(
      "Cell-Type-Specific Gradient Genes (red = induced near ",
      query_label, ")"
    ),
...
#'   Default: "mouse".
#' @param fgsea_min_genes Integer. Minimum number of genes per pathway for
...
run_ripple_atlas <- function(results_dir,
  ...
  .msg(
    "  Significant genes (", sig_col, " < ", fdr_threshold, "): ",
    nrow(sig_results)
  )
  ...
  tryCatch(
    {
      ...
      .panel_specific_heatmap(
        all_results, sig_results, gene_spec,
        coef_col, sig_col, fdr_threshold,
        query_label, f6
      )
      if (file.exists(f6)) {
      ...
      tryCatch(
        {
          ...
          classification_colors <- stats::setNames(
            ...
            c(
              query_specific_label, "enhanced", "niche_driven",
