# ============================================================================
# TDLN validation - FDC query with stromal subcluster annotation
# ============================================================================
# Identical setup to run_ripple_naive_fdc.R but subsets to TDLN samples
# (group == "TDLN", i.e. mice m14, m18, m19, m20) instead of naive.
#
# Rationale: TDLN samples have more active germinal centers than naive
# lymph nodes, so FDC-B cell biology (Cxcl13-Cxcr5 axis, CD83 light-zone
# signal, Bcl6 GC program) should be more pronounced. The per-cell
# transcriptional gradient for follicular markers in B_cell_all should
# therefore show stronger effect sizes and better sign consistency than
# the naive run.
#
# Query: FDC
# Expected biology in B_cell_all (should be STRONGER than naive):
#   Induced near FDC (negative coef):   Cxcr5, Bcl6, Cr2, Cd83
#   Repressed near FDC (positive coef): Gpr183 (EBI2, opposes Cxcr5)
#   Ambient-RNA check (produced by FDC, not B cell): Cxcl13
#
# Run with:
#   Rscript data-raw/xenium_tdln/run_ripple_tdln_fdc.R
#
# Output:
#   data-raw/xenium_tdln/tdln_fdc_ripple/
# ============================================================================
...
out_dir  <- "data-raw/xenium_tdln/tdln_fdc_ripple"
...
# --- 4. Subset to naive ---------------------------------------------------
cat("\nSubsetting to TDLN...\n")
tdln_cells <- rownames(obj@meta.data)[obj@meta.data$group == "TDLN"]
obj <- subset(obj, cells = tdln_cells)
cat("TDLN subset:", ncol(obj), "cells across",
    length(unique(obj@meta.data[[SAMPLE_COL]])), "mice\n")

md_n <- obj@meta.data

cat("\n=== Query (FDC) counts per TDLN mouse ===\n")
...
cat("\n=== B_cell_all counts per TDLN mouse ===\n")
